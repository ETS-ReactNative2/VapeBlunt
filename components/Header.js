...
export default class Header extends React.Component{
  ...
  renderMiddle = () =>{
    ...
    if(!searching && text){
      return (
        <Text style={{flex: 1, fontWeight: 'bold', fontSize: 23, color: colors.white, textAlign: 'center'}}>{text}</Text>
      )
    }else if(searching){
      return (
        <TextInput style={{flex: 1, fontSize: 17, backgroundColor: colors.white, marginVertical: 10, paddingVertical: 0}} />
      )
    }
    else{
      return(
  ...
  renderSearchIcon = () =>{
    ...
    if(searchBar){
      return(
        <TouchableOpacity onPress={() => this.toggleSearchBar()} style={{ alignItems: 'center' }} >
          <Image style={{height: 23, width: 23, marginLeft: 10}}
  ...
  render(){
    ...
    return (
      <View style={{flexDirection: "row", alignItems: 'center', height: 50, backgroundColor: color, paddingHorizontal: 10}}>
