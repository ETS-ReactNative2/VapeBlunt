...
const Carousel = (props) => {
  ...
  function scrollTo(index) {
    scrollViewRef.current.scrollTo({x: selfWidth * index});
    onPageChange(index);
  }
  ...
  return (
    <View onLayout={onLayout} style={{height: height, alignSelf: 'stretch', ...props.style}}>
      <ScrollView
        style={{
          alignSelf: 'stretch',
        ...
        {children.map((child, i) => (
          child && React.cloneElement(child, {style: { ...child.props.style, width: selfWidth}, key: 'a'+i})
        ))}
      </ScrollView>
    </View>
  );
